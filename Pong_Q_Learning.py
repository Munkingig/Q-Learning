...
class PongAgent:
	def __init__(self, game, policy = None, discount_factor = 0.1, learning_rate = 0.01, ratio_explotacion = 0.9):
	...
	def update(self, game, old_state, action_taken, reward_action_taken, new_state, reached_end):
		...
		if reached_end:
			future_max_q_value = reward_action_taken #Maxima recompensa
...
class PongEnvironment:
	...
	def __init__(self, max_life = 3, height_px = 40, width_px = 50, movimiento_px = 3):
		...
		self.player_1 = self.player_alto  #Posicion inicial del player
	...
	def _apply_action(self, action, animate = False):
		...
		self.state = (floor(self.player_1/abs(self.dy))-2, floor(self.y/abs(self.dy))-2, floor(self.x/abs(self.dx))-2)
	...
	def avanza_player(self):
		if self.player_1 + self.player_alto >= self.height_px:
	...
	def avanza_frame(self):
		...
		if self.x <= 3 or self.x > self.width_px:
			self.dx = -self.dx
			if self.x <= 3:
				ret = self.detectaColision(self.y, self.player_1)
	...
	def dibujar_frame(self):
		...
		a1.set_xlim(-5, self.widht_px + 5)

		rectangle = plt.Rectangle((-5, self.player1), 5, self.player_alto, fc='gold', ec="none")

		a1.add_patch(circle);
		a1.add_patch(rectangle)
		#a1.set_yticklabels([]);a1.set_xticlabels([])
		plt.text(4, self.height_px, "SCORE:" + str(self.total_reward)+"  LIFE"+str(self.lives), fontsize=12)
		if self.lives <= 0:
			plt.txt(10, self.height_px-14, "GAME OVER", fontsize=16)
		elif self.total_reward >= 1000:
			plt.text(10, self.height_px-14, "YOU MIN!", fontsize=16)
		return fig
...
def play(rounds = 5000, max_life = 3, discount_factor = 0.1, learning_rate = 0.1, ratio_explotacion= 0.9, learner = None, game = None, animate = False):
	...
	if game is None:
		# si usamos movimiento_px = 5 creamos una tabla de politicas de 8 x 10
